...
S&P 500 over a 30-day test. Research via Perplexity, alerts via WhatsApp (Twilio),
...
| daily-summary | 4:15 PM | `15 16 * * 1-5` | P&L, EOD snapshot, one WhatsApp recap |
...
scripts/               perplexity.sh (research), whatsapp.sh (notify)
...
- **Cloud routines** (this environment): schedule the four `routines/*.md` as triggers with
  the Robinhood connector + Perplexity/Twilio env vars attached. Simplest to start.
...
1. **Secrets** — `cp env.template .env`, fill in Perplexity + Twilio (local), or set them as
...
3. **Read-only verification** — run `routines/verify-readonly.md` as a scheduled trigger and
   confirm it reaches Robinhood and sends a WhatsApp — *in an unattended run*. This is the gate.
